...
def patch_text_cleaning(monkeypatch):
    ...
    monkeypatch.setattr("src.data_processing.preprocess_text",
                        fixed_preprocess_text)
...
def sample_df():
    return pd.DataFrame({
        "text": ["Hello WORLD!!!", "Test-case 2: Numbers 123.",
                        "Another_text -- special??"],
        "label": [0, 1, 0],
    })
...
def test_preprocess_and_tokenize(monkeypatch, sample_df):
    """Ensure preprocess_and_tokenize runs fully with mock tokenizer and
    dataset."""
    monkeypatch.setattr(
    ...
    def fake_map(self, func=None, batched=False, **kwargs):
        ...
        results = func({"text": texts}) if batched else [func({"text": t}) for
                                                         t in texts]
        if batched:
            data_dict = {k: list(v) if isinstance(v, list) else v for k, v in
                         self.to_dict().items()}
    ...
    train_ds, val_ds = preprocess_and_tokenize(sample_df.copy(),
                                               model_name="fake")
